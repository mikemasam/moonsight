...
UID.uuid = uuid;
UID.basic = basic;
UID.entity = (_type: string, seq: string | undefined) => {
  ...
  if (!opts.nodeIdentity) return [false, "Node ID not defined"];
  const RAND = ("" + Math.random()).slice(2, 6);
  //  02 000 2001 1231 4
  const checksum =
    (Number(E_TYPE) + Number(RAND) + Number(opts.nodeIdentity) + Number(seq)) %
    10;
...
UID.latestVersion = (version: any, minVersion: any) => {
