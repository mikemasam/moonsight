...
export const addISocketRoute = async (ctx, stat, isocket) => {
  ...
  if(!isocket.__ihandler) 
    throw `[KernelJs] ~ ${stat._fullPath} socket route doesn't return async ISocket handler.`;
  ...
  for(const middleware of middlewares){
    ...
    if(md?.action instanceof AsyncFn === false) 
      throw `[KernelJs] ~ ISocket Unknown middleware [${name}] ~ ${stat._location}`;
  ...
  if(opts.logging?.socket)
    console.log(`[KernelJs] ~ ISocket: ${stat._location}`, endpoint, `[${attached.join(',')}]`);
...
export const addISocketMount = async (ctx, stat, isocketmount) => {
  ...
  if(ctx.opts.logging?.socketmount)
    console.log(`[KernelJs] ~ ISocketMount: ${stat._location}`);
