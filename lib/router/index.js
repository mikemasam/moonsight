...
const addRoute = async (ctx, router, stat) => {
  ...
  const { opts } = ctx;
  const [ihttp, isocket, icore] = await loadRouteModule(ctx, stat);
  if(ihttp || isocket || icore){
    //console.log("--------------------------------------------------")
    await addIHttpRoute(ctx, router, stat, ihttp);
    await addISocketRoute(ctx, stat, isocket);
    await addICoreRoute(ctx, stat, icore);
    //console.log("--------------------------------------------------")
  }
}
