...
import { addISubRoute } from './sub-handler.js';
...
const counter = () => {
  return { icom: 0, isub: 0, ijob: 0, imount: 0, isocketmount: 0, ihttp: 0, ihttpmount: 0, icore: 0, isocket: 0 };
}
const addRoute = async (ctx, stat) => {
  ...
  for(let i = 0; i < routes.length; i++){
    ...
    if(route?.__ihandler == 'imount'){
      ...
      found.isub++;
      await addISubRoute(ctx, stat, route);
    }
  }
  for(let i = 0; i < routes.length; i++){
    const route = routes[i];
