...
export default async function CoreSocket(ctx, io){
  ...
  ctx.events.on("kernel.ready", () => {
    logger.kernel(`CoreNet: accepting [IP's Allowed ${allowedIPs}].`);
    coreIO.use((socket, next) => {
      if(allowedIPs.split(',').join('').split(' ').indexOf(socket.handshake.address) > -1){
        if(opts.logging.core)
          logger.kernel(`CoreNet: added [${socket.handshake.query.channelName} - ${socket.handshake.address}].`);
        next();
      }else{
        if(opts.logging.core)
          logger.kernel(`CoreNet: rejected [${socket.handshake.query.channelName} - ${socket.handshake.address}].`);
